...
const syncFileLoc = async function() {
  if (!fs.existsSync("./changes")) {
    ...
    await fsp.copyFile(
      "./src/assets/data/allfighters.json",
      "./changes/allfighters.json"
    )
    await fsp.copyFile(
      "./src/assets/data/backstories.json",
      "./changes/backstories.json"
    )

    const roundNumTest = /\d/ //there are only 8 rounds, don't need to be ready for double digits
    var roundNums = (await readAvailableRounds()).map(
      rf => rf.match(roundNumTest)[0]
    )
    // console.log(roundNums)
    for (const r of roundNums) {
      await fsp.copyFile(
        `./src/assets/data/rounds/round-${r}.json`,
        `./changes/round-${r}.json`
      )
    }
  }
}
...
module.exports = function(app) {
  ...
  app.post("/server/picture/:fighterId", async function(req, res) {
    try {
      ...
      await fs.writeFileSync(
        "./changes/allfighters.json",
        JSON.stringify(allfighters, null, 2)
      )
      res.status(200).send()
    } catch (err) {
      console.log(err)
  ...
  app.post("/server/artist/:fighterId", async function(req, res) {
    try {
      ...
      if (body.twitter && !(body.twitter === "")) {
        allfighters[fighterId].artists[body.artistname].contacts["twitter"] =
          body.twitter
      }

      if (body.instagram && !(body.instagram === "")) {
        allfighters[fighterId].artists[body.artistname].contacts["instagram"] =
          body.instagram
      }

      if (body.pw1 && !(body.pw1 === "")) {
        allfighters[fighterId].artists[body.artistname].contacts["pw1"] =
          body.pw1
      }

      if (body.pw2 && !(body.pw2 === "")) {
        allfighters[fighterId].artists[body.artistname].contacts["pw2"] =
          body.pw2
      }
    ...
    try {
      ...
      if (!("target" in req.body)) {
        ...
        res
          .status(400)
          .send(
            "the request must mention the target id (the fighter to survive the merge)"
          )
        return
      }
      if (!("doomed" in req.body)) {
        ...
        res
          .status(400)
          .send(
            "the request must include a field with the array (of id's) of fighters to remove"
          )
        return
      }
