...
for (const fighter of survey_data) {
  ...
  axios
    .post(`http://localhost:8080/server/fightername/${fighter.fighter_id}`, {
      name: fighter.combatant_name
    })
    .catch(err => {
      console.error(
        `rename failed for ${fighter.fighter_id} : ${fighter.combatant_name} : ${err}`
      )
    })

  //insert backstory
  axios
    .post(`http://localhost:8080/server/backstory/${fighter.fighter_id}`, {
      backstory: fighter.background
    })
    .catch(err => {
      console.error(
        `background failed for ${fighter.fighter_id} : ${fighter.combatant_name} : ${err}`
      )
    })
  ...
  await axios
    ...
    .catch(err => {
      console.error(
        `primary failed for ${fighter.fighter_id} : ${fighter.combatant_name} : ${err}`
      )
    })
  ...
  if (!regNocollab.test(fighter.collab_status)) {
    ...
    await axios
      ...
      .catch(err => {
        console.error(
          `collab failed for ${fighter.fighter_id} : ${fighter.combatant_name} : ${err}`
        )
      })
  }
}
