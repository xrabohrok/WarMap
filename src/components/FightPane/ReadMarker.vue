...
export default {
    ...
    methods: {
        toggleChecked: function(){
            ...
            if(this.checked) this.$store.commit(MARK_READ, {fighterId:this.fighterId, round:this.round})
            if(!this.checked) this.$store.commit(MARK_UNREAD, {fighterId:this.fighterId, round:this.round})
        }
    },
    computed:{
        ...mapGetters({
