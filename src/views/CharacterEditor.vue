<template>
  <div class="about">
    ...
    <div>
      <textarea class="backgrounds" v-model="curBackground"> </textarea>
    </div>

    <ProfilePicEditor
      :imgUrl="'/fighterimages/397.png'"
      :isEditable="true"
      class="profPic"
    />
  </div>
...
export default {
  ...
  methods: {
    ...
    async save() {
      axios
        .post("/server/mergeFighters", {
          target: this.selected[0],
          doomed: this.selected.slice(1)
        })
        .then(() => {
    ...
    async getBackground() {
      ...
      axios
        .get(`/server/backstory/${this.selected[0]}`)
